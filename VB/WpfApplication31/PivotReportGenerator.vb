...
Namespace WpfApplication31
    ...
    Public NotInheritable Class PivotReportGenerator
        ...
        Public Shared Function GenerateReport(ByVal pivot As PivotGridControl, ByVal kind As ReportGeneratorType, ByVal columnWidth As Decimal, ByVal repeatRowHeader As Boolean) As XtraReport
        ...
        Private Shared Sub FillDatasetExtracted(ByVal pivot As PivotGridControl, ByVal dataTable1 As DataTable)
            ...
            Dim fieldsInRowArea As List(Of PivotGridField) = GetFieldsInArea(pivot, FieldArea.RowArea)
            For i As Integer = 0 To pivot.RowCount - 1
                Dim pcea As PivotCellBaseEventArgs = pivot.GetCellInfo(0, i)
                If pcea.RowValueType = FieldValueType.Value Then
        ...
        Private Shared Sub FillDatasetColumns(ByVal pivot As PivotGridControl, ByVal dataTable1 As DataTable)
            ...
            Dim fieldsInColumnArea As List(Of PivotGridField) = GetFieldsInArea(pivot, FieldArea.ColumnArea)

            Dim multipleDataField As Boolean = pivot.GetFieldsByArea(FieldArea.DataArea).Count > 1
            For i As Integer = 0 To pivot.ColumnCount - 1
                Dim pcea As PivotCellBaseEventArgs = pivot.GetCellInfo(i, 0)
                ...
                End If
                If pcea.ColumnValueType = FieldValueType.Value Then
            ...
            Next i
        End Sub
        Private Shared Function GetFieldsInArea(ByVal pivot As PivotGridControl, ByVal area As FieldArea) As List(Of PivotGridField)
        ...
        Private Shared Function MeasureWidth(ByVal candidate As String, ByVal font As Font) As Single
            Dim formattedText As New FormattedText(candidate, CultureInfo.CurrentUICulture, FlowDirection.LeftToRight, New Typeface(font.Name), font.Size, System.Windows.Media.Brushes.Black)
            Dim width As Single = 0
            Single.TryParse(formattedText.Width.ToString(), width)
            Return width
        End Function
    End Class
End Namespace
